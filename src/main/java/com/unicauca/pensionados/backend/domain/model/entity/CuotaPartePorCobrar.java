...
/**
 * Entidad que representa las cuotas partes que otras entidades deben pagar
 * a la entidad principal (Universidad del Cauca) por pensionados que trabajaron
 * en múltiples instituciones.
 */
@Entity
@Table(name = "cuota_parte_por_cobrar", indexes = {
    @Index(name = "idx_cuota_cobrar_pensionado", columnList = "idPensionado"),
    @Index(name = "idx_cuota_cobrar_entidad_deudora", columnList = "idEntidadDeudora"),
    @Index(name = "idx_cuota_cobrar_periodo", columnList = "anioPeriodo, mesPeriodo"),
    @Index(name = "idx_cuota_cobrar_estado", columnList = "estado")
})
@Getter
@Setter
@NoArgsConstructor
...
public class CuotaPartePorCobrar {
    ...
    // Entidad que debe pagar (deudora)
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "idEntidadDeudora", nullable = false)
    private Entidad entidadDeudora;

    // Información de la entidad deudora (desnormalizada)
    @Column(name = "nitEntidadDeudora", nullable = false, length = 20)
    private String nitEntidadDeudora;

    @Column(name = "nombreEntidadDeudora", nullable = false, length = 200)
    private String nombreEntidadDeudora;
    ...
    @Column(name = "diasTrabajadosEntidad", nullable = false)
